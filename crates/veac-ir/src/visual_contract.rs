...
pub const MAX_VISUAL_SHEAR: f64 = 2.0;
...
pub fn visual_shear_valid(value: crate::Vec2) -> bool {
    value.x.is_finite()
        && value.y.is_finite()
        && (-MAX_VISUAL_SHEAR..=MAX_VISUAL_SHEAR).contains(&value.x)
        && (-MAX_VISUAL_SHEAR..=MAX_VISUAL_SHEAR).contains(&value.y)
}
