...
fn geometry_and_rate_accept_the_boundary_and_reject_the_next_value() {
    ...
    assert_limit(video(identity(1), 101, 1, 30), limits);
    assert_limit(video(identity(1), 101, 100, 30), limits);
...
fn exact_default_frame_and_pixel_work_boundaries_are_enforced() {
    ...
    assert_ok(video(identity(86_400), 1, 1, 30), limits);
    assert_limit(video(identity(86_400), 1, 1, 31), limits);
    assert_ok(video(identity(25_000), 1_600, 1_000, 100), limits);
    assert_limit(video(identity(25_000), 1_601, 1_000, 100), limits);
...
fn optical_flow_uses_the_same_bounded_video_work_contract() {
    let limits = MediaArtifactLimits {
        max_video_frames: 30,
        max_pixel_frames: 3_000,
        ..MediaArtifactLimits::default()
    };
    assert_ok(flow(10, 10, 30), limits);
    assert_limit(flow(10, 10, 31), limits);
    assert_limit(flow(11, 10, 30), limits);
}
