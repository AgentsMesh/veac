...
fn honors_precedence_parentheses_and_unary_signs() {
    ...
    assert_eq!(value("-250ms / -2").render(), "125ms");
...
fn distinguishes_hyphenated_symbols_from_subtraction() {
    ...
    assert_eq!(evaluate("title-size", &environment).unwrap().render(), "4");
    assert_eq!(
    ...
    assert_eq!(evaluate("foo--bar", &environment).unwrap().render(), "4");
...
fn applies_strict_dimension_arithmetic() {
    ...
    assert_eq!(value("2 * 3deg").render(), "6deg");
    assert_eq!(value("3deg * 2").render(), "6deg");
    assert_eq!(value("80% * 2").render(), "160%");
    assert_eq!(value("2 * 80%").render(), "160%");
    assert_eq!(value("50% * 50%").render(), "25%");
    assert_eq!(value("200px * 25%").render(), "50px");
    assert_eq!(value("25% * 200px").render(), "50px");
    assert_eq!(value("8s / 2").render(), "4s");
    assert_eq!(value("8s / 25%").render(), "32s");
    assert_eq!(value("8s / 2s").render(), "4");
    assert_eq!(value("1 / 25%").render(), "4");
    assert_eq!(value("50% / 2").render(), "25%");
}
...
fn rejects_invalid_operator_and_function_types() {
    ...
    assert_eq!(error("min(1, 1s)").code(), "EXPRESSION_TYPE");
    assert_eq!(error("min(true, false)").code(), "EXPRESSION_TYPE");
...
fn text_concatenation_has_a_deterministic_output_budget() {
    let environment = [
        ("left".to_owned(), Value::Text("a".repeat(600_000))),
        ("right".to_owned(), Value::Text("b".repeat(600_000))),
    ]
    .into_iter()
    .collect();
    assert_eq!(
    ...
    let oversized = [("value".to_owned(), Value::Text("x".repeat(1024 * 1024 + 1)))]
        .into_iter()
        .collect();
    assert_eq!(
