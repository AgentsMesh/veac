...
#[test]
fn contract_rejects_version_operation_id_node_id_and_named_site() {
    let mut value = batch();
    value.schema_version = 2;
    assert_eq!(
        validate_source_edit_contract(&value),
        Err(SourceEditError::UnsupportedSchemaVersion(2))
    );
    ...
    value.operations = vec![SourceEditOperation::SetExpression {
        target: SourceNodeRef::item("timeline/main.veac", "timeline", "main", "visual", "bad.id"),
    ...
    value.operations = vec![SourceEditOperation::SetExpression {
        ...
        site: ExpressionSite::ModifierParameter {
...
fn source_targets_use_the_canonical_language_name_contract() {
    for valid in ["_private", "title-card", "name_2"] {
        let mut value = batch();
        value.operations = vec![SourceEditOperation::SetExpression {
            target: SourceNodeRef::item("main.veac", "show", "main", "visual", valid),
            site: ExpressionSite::ItemEnabled,
            expression: ExpressionSource {
                source: "true".to_owned(),
            },
    ...
    for invalid in [
        ...
        let mut value = batch();
        value.operations = vec![SourceEditOperation::SetExpression {
            target: SourceNodeRef::item("main.veac", "show", "main", "visual", invalid),
            site: ExpressionSite::ItemEnabled,
            expression: ExpressionSource {
                source: "true".to_owned(),
            },
...
#[test]
fn contract_requires_exactly_one_pure_expression() {
    for source in [
