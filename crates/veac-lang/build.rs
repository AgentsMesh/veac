...
fn main() {
    run().unwrap_or_else(|message| panic!("compiler build identity failed: {message}"));
}
...
fn run() -> Result<(), String> {
    ...
    let workspace = manifest
        ...
        .ok_or_else(|| "veac-lang must be nested under workspace/crates".to_owned())?;
    let mut paths = fixed_paths(workspace, &manifest);
    collect_rs(&manifest.join("src"), &mut paths)?;
    collect_rs(&workspace.join("crates/veac-ir/src"), &mut paths)?;
    paths.sort();
    paths.dedup();
    ...
    let files = owned
        ...
        .collect::<Vec<_>>();
    let source = build_identity::source_fingerprint(&files)?;
    let rustc = rustc_identity()?;
    let target = required("TARGET")?;
    let features = enabled_features();
    let build = build_identity::build_fingerprint(&BuildInputs {
    ...
    println!("cargo:rustc-env=VEAC_COMPILER_SOURCE_SHA256={source}");
    println!("cargo:rustc-env=VEAC_COMPILER_BUILD_SHA256={build}");
    Ok(())
}
