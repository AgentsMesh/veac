...
const DOMAIN: &str = "veac.runtime";

fn main() {
    run().unwrap_or_else(|message| panic!("runtime build identity failed: {message}"));
}
...
fn run() -> Result<(), String> {
    ...
    let workspace = manifest
        ...
        .ok_or_else(|| "veac-runtime must be nested under workspace/crates".to_owned())?;
    ...
    println!("cargo:rustc-env=VEAC_RUNTIME_SOURCE_SHA256={source}");
    println!("cargo:rustc-env=VEAC_RUNTIME_BUILD_SHA256={build}");
    Ok(())
}
...
fn fixed_paths(workspace: &Path, manifest: &Path) -> Vec<PathBuf> {
    [
        ...
        workspace.join("crates/veac-codegen/Cargo.toml"),
