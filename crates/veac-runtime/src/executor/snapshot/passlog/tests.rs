...
fn expired_passlog_copy_preserves_resource_limit_kind() {
    ...
    let error =
        ReboundPasslogs::capture(&task, &[path], &[record(b"log")], std::time::Instant::now())
            .unwrap_err();
...
fn rejects_incomplete_and_non_second_pass_contracts() {
    ...
    assert!(
        ReboundPasslogs::capture(&single, &[path.clone()], &[record.clone()], deadline()).is_err()
    );
    ...
    assert!(
        ReboundPasslogs::capture(&not_ffmpeg, &[path], &[record], deadline())
            .unwrap_err()
            .message
...
fn rejects_wrong_family_size_and_identity() {
    ...
    assert!(
        ReboundPasslogs::capture(&task, &[foreign], &[record(b"log")], deadline())
            .unwrap_err()
            .message
    ...
    assert!(
        ReboundPasslogs::capture(&task, &[log.clone()], &[wrong_size], deadline())
            .unwrap_err()
            .message
            .contains("size changed")
    );
    assert!(
        ReboundPasslogs::capture(&task, &[log], &[record(b"other")], deadline())
            .unwrap_err()
            .message
            .contains("identity")
    );
