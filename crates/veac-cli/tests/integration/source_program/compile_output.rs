...
fn assert_compile_rejects(entry: &std::path::Path, output: &std::path::Path, code: &str) {
    veac()
        .args([
            "compile",
            entry.to_str().unwrap(),
...
#[test]
fn compile_cannot_overwrite_an_imported_source_module() {
    let (_temp, entry, module) = fixture();
    let original = std::fs::read(&module).unwrap();
    assert_compile_rejects(&entry, &module, "OUTPUT_OVERWRITES_INPUT");
    assert_eq!(std::fs::read(module).unwrap(), original);
}

#[test]
fn compile_cannot_overwrite_a_hardlink_to_an_imported_module() {
    let (temp, entry, module) = fixture();
    let alias = temp.path().join("module-alias.json");
    std::fs::hard_link(&module, &alias).unwrap();
    let original = std::fs::read(&module).unwrap();
    assert_compile_rejects(&entry, &alias, "OUTPUT_OVERWRITES_INPUT");
    assert_eq!(std::fs::read(module).unwrap(), original);
}

#[cfg(unix)]
#[test]
fn compile_rejects_a_symlink_output_before_writing() {
    ...
    let original = std::fs::read(&module).unwrap();
    assert_compile_rejects(&entry, &alias, "OUTPUT_SYMLINK");
    assert_eq!(std::fs::read(module).unwrap(), original);
}
