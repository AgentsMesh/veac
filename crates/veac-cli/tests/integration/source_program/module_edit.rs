...
fn source_edit_updates_the_target_module_and_protects_the_source_graph() {
    ...
    let SourceEditOperation::SetExpression { target, .. } = &mut edit.operations[0] else {
        panic!("fixture must remain an expression edit");
    };
    ...
    veac()
        ...
        .stdout(predicate::str::contains("\"brand.veac\""));
    ...
    let SourceEditOperation::SetExpression { target, .. } = &mut next.operations[0] else {
        panic!("fixture must remain an expression edit");
    };
    target.module = "brand.veac".into();
    std::fs::write(
