...
fn inline_ass_and_final_filter_stay_below_linux_single_argument_limit() {
    ...
    text_content(&mut plan).styled_mut().unwrap().background = None;
...
fn untrusted_text_and_surface_budgets_fail_before_font_loading() {
    ...
    content.styled_mut().unwrap().layout.box_width_pixels = Some(MAX_TEXT_BOX_DIMENSION);
    content.styled_mut().unwrap().layout.box_height_pixels = Some(MAX_TEXT_BOX_DIMENSION);
