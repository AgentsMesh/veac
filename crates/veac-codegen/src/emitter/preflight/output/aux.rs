use veac_plan::canonical::{
    AudioCodec, AudioMixSource, AudioStemFormat, CaptionSidecarFormat, Deliverable,
...
pub(super) fn validate(check: &mut Check, plan: &ResolvedRenderPlan, value: &Deliverable) {
    ...
    let (valid, code, message) = match &value.kind {
        ...
        | DeliverableKind::AdaptivePackage(_) => {
            unreachable!("extended deliveries have a dedicated preflight")
        }
    };
    if !valid {
...
fn stem(
    ...
    value
        .target
        .file_name()
        .is_some_and(|name| extension_is(name, extension))
