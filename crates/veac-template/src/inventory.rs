use veac_ir::{
    ClipSource, Frame, ItemId, MaterialId, ProjectEnvelope, SlotConstraint, SlotKind, TrackId,
};
    ...
    for sequence in &project.project.sequences {
        ...
        for track in &sequence.tracks {
            for clip in &track.clips {
                match (&clip.replaceable, &clip.source) {
                    (Some(constraint), ClipSource::Media { material_id })
                        if constraint.kind != SlotKind::Text =>
                    {
                    ...
                    (Some(constraint), ClipSource::Text { .. })
                        if constraint.kind == SlotKind::Text && clip.template_editable_text =>
                    {
