...
use super::super::{EvidenceDecodeError, EVIDENCE_MODULE_ID};
...
impl<'a> Decoder<'a> {
    ...
    pub fn structure(
        ...
        name: &str,
        path: &str,
    ) -> Result<Fields<'a>, EvidenceDecodeError> {
        ...
        let TypeDefinitionKind::Struct(layout) = definition.kind() else {
            return Err(EvidenceDecodeError::new(
                path,
    ...
    pub fn variant(
        ...
        name: &str,
        path: &str,
    ) -> Result<Variant<'a>, EvidenceDecodeError> {
        ...
        let variant = layout
            ...
            .ok_or_else(|| {
                EvidenceDecodeError::new(path, format!("{name} has an invalid variant index"))
            })?;
        Ok(Variant {
            name: variant.name(),
    ...
    pub fn list_map<T>(
        ...
        value: &'a Value,
        path: &str,
        decode: fn(&Decoder<'_>, &Value, &str) -> Result<T, EvidenceDecodeError>,
    ) -> Result<Vec<T>, EvidenceDecodeError> {
    ...
    fn definition<V: NominalValue>(
        ...
        name: &str,
        path: &str,
    ) -> Result<&'a TypeDefinition, EvidenceDecodeError> {
        let definition = self.types.definition(value.type_id()).ok_or_else(|| {
            EvidenceDecodeError::new(path, "nominal value type is absent from the registry")
        })?;
        if definition.canonical_source_id() != EVIDENCE_MODULE_ID
            ...
            || definition.digest() != value.definition_digest()
        {
            return Err(EvidenceDecodeError::new(
                path,
                format!(
                    "expected nominal {EVIDENCE_MODULE_ID}::{name}, got {}::{}",
...
impl<'a> Fields<'a> {
    pub fn get(&self, name: &str) -> Result<&'a Value, EvidenceDecodeError> {
        let field = self
            .layout
            .iter()
            .find(|field| field.name() == name)
            .ok_or_else(|| {
                EvidenceDecodeError::new(
                    self.path(name),
                    "ABI field is missing from its definition",
                )
            })?;
        self.values
            .get(field.index().index())
            .ok_or_else(|| EvidenceDecodeError::new(self.path(name), "ABI field value is missing"))
    }
...
pub(super) fn unknown_variant(path: &str, ty: &str, name: &str) -> EvidenceDecodeError {
    EvidenceDecodeError::new(path, format!("unknown {ty} variant `{name}`"))
}

fn kind(path: &str, expected: &str, value: &Value) -> EvidenceDecodeError {
    EvidenceDecodeError::new(
        path,
