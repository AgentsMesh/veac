...
payload 的 nominal enum。enum 可以声明在 root entry，也可以从 module 导入；struct、带 payload
variant 的 enum 和 collection 不能作为 Build input。它们进入 typed HIR 时已经解析为 slot；verified
